    ...
    def loopChatter(self):
        while 1:
            try:
                ...
                globals.data.activeUsersOverTime[time.strftime("%d:%H:%M")] = max(0, (len(self.mods) + len(self.viewers)) - 1) # -1 because bot is always a user
                globals.data.save()
...
class irc:
    ...
    def chatTimeoutCheck(self):
        ...
        if (self.chat_time == self.chat_timeout_max):
            self.msg(
                "There has not been any activity for the past %s minutes, pybot is now disconnecting from your chat." % self.chat_timeout_max,
                "log")
            self.close()
        else:
                    ...
                    if msg == "ERROR":
                        ...
                        if ("jtv.tmi.twitch.tv PRIVMSG " + self.channel not in msg):
                            ...
                            if ("SPECIALUSER" in msg):  # :jtv PRIVMSG pybot_beta :SPECIALUSER username subscriber
                                ...
                                text = msg.split("PRIVMSG")[1].replace('%s :' % self.channel, '').replace(
    ...
    def isMod(self, name):
        if (name.lower() == self.channel.replace('#',
                                                 '').lower()):  # if name is host its auto mod, comment this out if you want to test filters on your own channel
            return True
        else:
            try:
